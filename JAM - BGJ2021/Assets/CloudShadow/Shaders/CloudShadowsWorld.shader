Shader "Projector/CloudShadowsWorld" {
	Properties {
		...
		[NoScaleOffset] _NoiseTex ("Distortion Texture", 2D) = "black" { }
	 ...
	 Subshader {
		...
		Pass {
			...
			fixed4 frag (v2f i) : COLOR {
				float4 noiseDistortion = tex2D(_NoiseTex, (float2(i.wPos.x*_NoiseTiling*0.0025f+frac(_Time.x*-_SpeedX),i.wPos.z*_NoiseTiling*0.0025f+frac(_Time.x*-_SpeedY)))); // The 0.0025 value here is arbitrary, for easier upgrading and adjustment in the inspector.
				fixed4 c = tex2D(_ShadowTex, fixed2(i.wPos.x*_ShadowTiling*0.0025f + noiseDistortion.x + frac(_Time.x*-_SpeedX),i.wPos.z*_ShadowTiling*0.0025f + noiseDistortion.y + frac(_Time.x*-_SpeedY)));  
